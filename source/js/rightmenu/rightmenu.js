const RightMenus = {
    ...
    messageRightMenu: stellar.GLOBAL_CONFIG.plugins.message.enable && stellar.GLOBAL_CONFIG.plugins.message.rightmenu.enable,
    corsAnywhere: stellar.GLOBAL_CONFIG.rightmenu.options.corsAnywhere,
    ...
    initialMenu: () => {
      RightMenus.fun.init();
      if (!stellar.dom.mask.classList.contains('active')) stellar.dom.mask.style.display = 'none';
    },
   ...
   */
  RightMenus.fun = (() => {
    const rightMenuConfig = stellar.GLOBAL_CONFIG.rightmenu;
    ...
    fn.menuPosition = (event) => {
      try {
        ...
        if (stellar.GLOBAL_CONFIG.plugins.message.rightmenu.notice) fn.menuNotic();
      } catch (error) {
        console.error(error);
        fn.hideMenu();
        return true;
    ...
    fn.menuControl = (event) => {
      ...
      _rightMenuList.forEach(item => {
        ...
        if (globalData.statusCheck || globalData.isArticle || globalData.isWiki) {
          switch (groupName) {
            case 'inputBox':
            ...
            case 'articlePage':
              ...
              break;
            case 'prevNext':
              if (globalData.isArticle || globalData.isWiki) {
                let article_prev_item = document.querySelector('#read-next .body #prev.active');
                let article_next_item = document.querySelector('#read-next .body #next.active');
                let prev_item = document.querySelector('.menuLoad-Content #prev');
                let next_item = document.querySelector('.menuLoad-Content #next');
                prev_item.parentElement.style.display = article_prev_item ? 'block' : 'none';
                next_item.parentElement.style.display = article_next_item ? 'block' : 'none';
              }
              break;
            default:
              item.style.display = nodeName === 'A'
                ? (globalData.isArticle || globalData.isWiki) && !globalData.statusCheck && rightMenuConfig.options.articleShowLink
                  ? 'block'
                  : 'none'
                : 'block';
              break;
      ...
      // 执行外部事件
      stellar.mouseEvent = event;
      stellar.rightmenu.method.handle.start()
    ...
    fn.globalDataSet = (event) => {
      ...
      // 判断是否为wiki页面
      if (!!(document.querySelector('article.wiki') || null)) {
        globalData.isWiki = true;
      }
  
      // 判断是否显示音乐控制器
      if (stellar.GLOBAL_CONFIG.plugins.aplayer?.enable
    ...
    fn.contextmenu = () => {
      window.document.oncontextmenu = (event) => {
        ...
        stellar.dom.mask.style.display = 'flex';
    ...
    fn.mobileMenu = () => {
      if(window.navigator.userAgent.match(/(phone|pad|pod|iPhone|iPod|ios|iPad|Android|Mobile|BlackBerry|IEMobile|MQQBrowser|JUC|Fennec|wOSBrowser|BrowserNG|WebOS|Symbian|Windows Phone)/i)) {
        ...
        addLongtabListener(box, fn.contextmenu)
      }
    }
      ...
      _rightMenuList.forEach(item => {
        ...
        item.addEventListener('click', () => {
          try {
            ...
            if (stellar.GLOBAL_CONFIG.debug === "rightMenus") {
              console.error({
                id: id,
                error: error,
    ...
    fn.menuNotic = () => {
      ...
      if (RightMenus.messageRightMenu && !NoticeRightMenu)
        hud.message('右键菜单', '唤醒原系统菜单请使用：<kbd>Ctrl</kbd> + <kbd>右键</kbd>', {
          icon: rightMenuConfig.options.iconPrefix + ' fa-exclamation-square red',
          displayMode: 1,
          time: 9000
        }, () => {
    ...
    fn.copyText = () => {
      util.utilWriteClipText(globalData.selectText)
        .then(() => {
          if (RightMenus.messageRightMenu) {
    ...
    fn.copyLink = () => {
      util.utilWriteClipText(globalData.linkUrl || globalData.mediaLinkUrl)
        .then(() => {
          if (RightMenus.messageRightMenu) {
            util.messageCopyright();
          }
    ...
    fn.copyPaste = async () => {
      ...
      if (RightMenus.messageRightMenu && window.clipboardRead === false) {
        hud.message('系统提示', '未授予剪切板读取权限！');
      } else if (RightMenus.messageRightMenu && result === '') {
        hud.message('系统提示', '仅支持复制文本内容！');
      } else {
    ...
    fn.copyImg = () => {
      if (stellar.GLOBAL_CONFIG.plugins.message.rightmenu.notice) {
        hud.message('系统提示', '复制中，请等待。', {
          icon: rightMenuConfig.options.iconPrefix + ' fa-images'
        })
      }
      RightMenus.writeClipImg(globalData.mediaLinkUrl, e => {
        if (RightMenus.messageRightMenu) {
          stellarApp.hideMessage();
          hud.message('系统提示', '图片复制成功！', {
            icon: rightMenuConfig.options.iconPrefix + ' fa-images'
          });
        }
      }, (e) => {
    ...
    fn.printMode = () => {
      if (window.location.pathname === globalData.pathName) {
        if (RightMenus.messageRightMenu) {
          ...
          stellarApp.question('', message, { time: 9000 }, () => { fn.printHtml() })
        } else {
          fn.printHtml()
        }
      }
    ...
    fn.printHtml = () => {
      if (stellar.isReadModel) fn.readMode();
      DOMController.setAttribute('details', 'open', 'true');
      DOMController.removeList([
        '.cus-article-bkg', '.iziToast-overlay', '.iziToast-wrapper', '#read-next',
        'footer', '#l_header', '#l_cover', '.l_left', '#comments', '#s-top', '#BKG',
    ...
    fn.readMode = () => {
      ...
      DOMController.fadeToggleList([
        ...
        document.querySelector('#read-next'), document.querySelector('.l_left'),
      ...
      stellar.isReadModel = stellar.isReadModel === undefined ? true : !stellar.isReadModel;
      if (stellar.isReadModel) {
        if (RightMenus.messageRightMenu) hud.message('系统提示', '阅读模式已开启，您可以点击屏幕空白处退出。', {
          backgroundColor: 'var(--block)',
          icon: rightMenuConfig.options.iconPrefix + ' fa-book-reader PETERRIVE',
          displayMode: 1,
          time: 5000
        });
        ...
        DOMController.setStyle('#read-next', 'display', 'flex');
    ...
    fn.prev = () => {
      let prev_item = document.querySelector('#read-next .body #prev.active');
      if(!!prev_item) {
        window.location.href = prev_item.href;
      }
    }

    fn.next = () => {
      let next_item = document.querySelector('#read-next .body #next.active');
      if(!!next_item) {
        window.location.href = next_item.href;
      }
    }
