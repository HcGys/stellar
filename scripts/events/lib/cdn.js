...
// stellar_cdn_system_prefix
// 在本配置文件中 匹配以 "stellar-local/npm/static/cdnjs" 开头的链接路径替换为该格式的前缀开头 prefix
/**
    ...
    if (isObject(source[key])) {
      ...
      source[key].forEach((item, index) => {
        if (item && typeof item == "string") {
          for (const prefix of prefix_list) {
            if (item.match(new RegExp(`^stellar-${prefix}\/`, "g"))) {
              const my_prefix = hexo.theme.config.cdn_system.system_config[prefix].prefix.replace(/\$\{site_root\}/g, site_root)
              source[key][index] = my_prefix + item.replace(new RegExp(`^stellar-${prefix}`, "g"), "")
              break;
            }
          }
        } else if (isObject(item)) {
          stellar_cdn_system_prefix(item, hexo);
        }
      })
    }
  }
...
hexo.on('generateBefore', () => {
  ...
  stellar_cdn_system_prefix(hexo.theme.config, hexo);
